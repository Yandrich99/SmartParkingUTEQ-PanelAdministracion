...
Este repositorio es una práctica universitaria construida sobre esta plantilla (React + Vite + CoreUI). Implementa el panel de administración del caso de estudio **UTEQ Smart Parking**, con tres vistas propias bajo `src/views/parqueadero/`:

- **Vehículos y propietarios** (`/parqueadero/vehiculos`) — alta, edición, baja y búsqueda de vehículos y sus propietarios en Supabase.
- **Puestos** (`/parqueadero/puestos`) — estado de los puestos de estacionamiento.
- **Historial de parqueo** (`/parqueadero/historial`) — registros de entradas y salidas.
- **Monitoreo de entrada** (`/parqueadero/monitoreo-entrada`) — captura una foto por cámara o archivo, la envía a un endpoint REST de reconocimiento de placas (OCR) y muestra si el vehículo está registrado y autorizado a ingresar.

### Variables de entorno

Copie `.env.local.example` a `.env.local` y complete sus propios valores (este archivo está ignorado por git):
...
`VITE_OCR_ENDPOINT` es la URL completa (incluido su `?code=...`) del endpoint de reconocimiento de placas proporcionado por el docente. Nunca se escribe directamente en el código ni se sube al repositorio.

### Despliegue en Azure Static Web Apps

El workflow `.github/workflows/azure-static-web-apps.yml` compila el proyecto con `npm run build` (usando las variables de entorno como secretos de GitHub Actions) y sube el resultado a Azure Static Web Apps, que expone la app por HTTPS — requisito para poder usar la cámara del dispositivo.

Pasos para configurarlo:

1. En Azure Portal, cree un recurso **Static Web App** (plan gratuito), eligiendo "Other" como origen de despliegue (no conecte el repositorio desde el asistente, ya que este repo ya trae su propio workflow).
2. Copie el **deployment token** del recurso creado (Overview → "Manage deployment token").
3. En GitHub, vaya a *Settings → Secrets and variables → Actions* del repositorio y cree los siguientes secretos:
   - `AZURE_STATIC_WEB_APPS_API_TOKEN` → el deployment token del paso anterior.
   - `VITE_SUPABASE_URL`
   - `VITE_SUPABASE_PUBLISHABLE_KEY`
   - `VITE_OCR_ENDPOINT`
4. Haga `push` a la rama `main`. El workflow compilará y desplegará automáticamente; la URL pública queda visible en el recurso de Azure Portal.
